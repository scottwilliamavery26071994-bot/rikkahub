    ...
    suspend fun backup(config: WebDavConfig) = withContext(Dispatchers.IO) {
        ...
        runCatching {
            ...
            if (items.size > MAX_BACKUPS) {
                items.drop(MAX_BACKUPS).forEach { deleteBackupFile(config, it) }
                Log.i(TAG, "backup: rotation removed ${items.size - MAX_BACKUPS} old backups")
            }
        }.onFailure { e ->
            Log.w(TAG, "backup: rotation failed", e)
        }
    }
