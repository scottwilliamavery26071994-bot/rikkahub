...
private fun rememberReasoningState(reasoning: UIMessagePart.Reasoning): Pair<ReasoningState, Boolean> {
    ...
    // 跟随布局变化滚动到底部（用 scrollTo 瞬间定位，避免 animateScrollTo 动画叠加导致抽搐）
    LaunchedEffect(Unit) {
...
private fun ReasoningContent(
    ...
    Column(
        ...
        if (loading) {
            // 流式生成期间不启用文本选择：避免 Compose SelectionContainer
            // 在内容高频变化时 selection 越界崩溃（getPathForRange start > end）
            MarkdownBlock(
...
fun ChainOfThoughtScope.ChatMessageReasoningStep(
    ...
    ControlledChainOfThoughtStep(
        ...
        extra = {
            if (state.duration > 0.seconds) {
                val displaySettings = me.rerere.rikkahub.ui.context.LocalDisplaySettings.current
                if (displaySettings.showThinkingTime) {
                    Text(
