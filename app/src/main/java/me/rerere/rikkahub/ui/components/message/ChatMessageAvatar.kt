    ...
    if (message.role == MessageRole.ASSISTANT && (model != null || useAssistantAvatar)) {
        Row(
            ...
            if (useAssistantAvatar) {
                ...
                // 模型可能已被从供应商删除（model 为 null），
                // 仍显示助手名作为发言人标识，避免消息"匿名"（#1006 #1144）
                val fallbackName = assistant?.name?.ifEmpty {
                    stringResource(R.string.assistant_page_default_assistant)
                } ?: stringResource(R.string.assistant_page_default_assistant)
                if (showIcon) {
                    Box(contentAlignment = Alignment.Center) {
                        AutoAIIcon(
                            name = model?.modelId ?: fallbackName,
                            modifier = Modifier.size(32.dp),
                            loading = loading
                        )
                        AvatarFrameOverlay(
                ...
                Column(
                    ...
                    if(settings.displaySetting.showModelName) {
                        Text(
                            text = model?.displayName ?: fallbackName,
                            style = MaterialTheme.typography.titleSmallEmphasized,
                        )
                        if (settings.displaySetting.showDateBelowName) {
                            Text(
