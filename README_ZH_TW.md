...
### 🛠️ 擴展能力
- **API 探索器** — 輸入網址自動發現 API 接口
- **APK 逆向分析** — 選擇 APK 檔案，分析包名/權限/介面，反編譯 Java 原始碼
- **RailGo 火車查詢** — 查車站、查車次、看時刻表
- **五子棋** — 15x15 棋盤，人機對戰
- **工具箱** — Base64/時間戳/密碼/JSON/顏色/進制/正則
- **語音通話** — AI 語音對話（VoIP）
- **SSH 遠端連線** — 工作區終端 SSH/SFTP
- **安全審計日誌** — 記錄敏感操作與工具呼叫
- **記憶庫** — AI 長期記憶儲存與檢索
- **主動訊息** — AI 定時主動推送，有記憶有上下文
- **微信 Bot** — 把微信號變成 AI 入口
- **工作流** — Tasker 風格自動化：觸發器 + 條件 → 執行動作
- **多供應商支援** — OpenAI / Claude / Gemini / DeepSeek / 智譜 / 通義千問 等
- **MCP 協議支援** — 連接外部工具與服務
