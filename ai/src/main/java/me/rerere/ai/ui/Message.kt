...
data class UIMessage(
     ...
     * 群聊模式下是否对全体成员可见：
     * - true  = 公开（所有 AI 都能看到）
     * - false = 私有（仅产生该回复的 AI 自己可见，需用户手动公开）
     * 单聊不受影响（默认 true）。
     */
    val isPublic: Boolean = true,
    /** 是否为助手预设消息（开场白等），时间提醒等逻辑应忽略 */
                ...
                when (deltaPart) {
                    is UIMessagePart.Text -> {
                        ...
                        if (deltaPart.text.isEmpty()) {
                            ...
                            val lastTextIdx = acc.indexOfLast { it is UIMessagePart.Text }
                            if (lastTextIdx >= 0) {
                                val lastText = acc[lastTextIdx] as UIMessagePart.Text
                                acc.toMutableList().apply {
                                    set(lastTextIdx, lastText.copy(text = lastText.text + deltaPart.text))
                                }
                            } else {
                    ...
                    is UIMessagePart.Reasoning -> {
                        ...
                        if (deltaPart.reasoning.isEmpty() && deltaPart.metadata == null) {
                            ...
                            val lastReasoningIdx = acc.indexOfLast { it is UIMessagePart.Reasoning }
                            if (lastReasoningIdx >= 0) {
                                val lastReasoning = acc[lastReasoningIdx] as UIMessagePart.Reasoning
                                acc.toMutableList().apply {
                                    set(lastReasoningIdx, UIMessagePart.Reasoning(
                                        reasoning = lastReasoning.reasoning + deltaPart.reasoning,
                                        createdAt = lastReasoning.createdAt,
                                        finishedAt = null,
                                    ).also {
                                        it.metadata = deltaPart.metadata ?: lastReasoning.metadata
                                    })
                                }
                            } else {
