...
fun ProviderBalanceText(
    ...
    val value = produceState(initialValue = "~", key1 = providerSetting.id, key2 = providerSetting.balanceOption) {
        // Check cache first
        val cachedBalance = cache["${providerSetting.id},${providerSetting.balanceOption.hashCode()}"]
        if (cachedBalance != null) {
            ...
            runCatching {
                ...
                // Cache the result
                cache["${providerSetting.id},${providerSetting.balanceOption.hashCode()}"] = balance
                value = balance
            }.onFailure {
                // Handle error
