...
class RootfsInstaller(
    ...
    internal fun extractTar(
        ...
        // 硬链接延迟创建：解压期间只记录映射，等所有普通文件解压完成后再统一创建。
        // 避免 tar 包中硬链接条目出现在源文件之前导致源不存在而丢失/降级复制，
        // 也便于对同一源去重（文件系统不支持硬链接时只复制一份，其余用符号链接）。
        ...
        format.wrapStream(BufferedInputStream(archive.inputStream())).use { input ->
            ...
            while (true) {
                ...
                when (header.type) {
                    ...
                    TarEntryType.HARDLINK -> pendingHardLinks.add(target to header.linkName)
                    TarEntryType.FILE -> {
                        target.outputStream().use { output ->
                ...
                input.skipFully(header.size.paddingSize())
                if (header.modTime > 0 && header.type != TarEntryType.SYMLINK && header.type != TarEntryType.HARDLINK) {
        ...
        // 解压完成后统一创建硬链接（源文件此时必然已就位）
        createHardLinksDeferred(targetDir, pendingHardLinks)
    }

    /**
     * 延迟创建硬链接：
     * 1. 源存在且文件系统支持硬链接 → Files.createLink（零额外空间）
     * 2. createLink 失败（如文件系统不支持）→ 目标创建符号链接指向源文件（源本身已解压，零额外空间）
     * 3. 符号链接也失败 → 逐份复制（极端兜底）
     */
    private fun createHardLinksDeferred(root: File, pending: List<Pair<File, String>>) {
        for ((target, linkName) in pending) {
            if (linkName.isBlank()) continue
            val source = runCatching { root.safeResolve(linkName) }.getOrNull() ?: continue
            ...
            if (created) continue
            // 文件系统不支持硬链接：创建符号链接指向源（源为唯一真实数据，不产生额外副本）
            val symlinked = runCatching {
                createSymlink(root, target, source.absolutePath)
            }.isSuccess
            if (!symlinked) {
                // 符号链接也失败（极端文件系统）：复制一份兜底
                runCatching {
                    ...
                    Log.w(TAG, "createHardLinksDeferred: fallback copy failed $source → $target", it)
                }
            }
        }
    }
