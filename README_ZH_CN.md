...
### 🛠️ 扩展能力
- **API 探索器** — 输入网址自动发现 API 接口
- **APK 逆向分析** — 选择 APK 文件，分析包名/权限/接口，反编译 Java 源码
- **RailGo 火车查询** — 查车站、查车次、看时刻表
- **五子棋** — 15x15 棋盘，人机对战
- **工具箱** — Base64/时间戳/密码/JSON/颜色/进制/正则
- **语音通话** — AI 语音对话（VoIP）
- **SSH 远程连接** — 工作区终端 SSH/SFTP
- **安全审计日志** — 记录敏感操作与工具调用
- **记忆库** — AI 长期记忆存储与检索
- **主动消息** — AI 定时主动推送，有记忆有上下文
- **微信 Bot** — 把微信号变成 AI 入口
- **工作流** — Tasker 风格自动化：触发器 + 条件 → 执行动作
- **多供应商支持** — OpenAI / Claude / Gemini / DeepSeek / 智谱 / 通义千问 等
- **MCP 协议支持** — 连接外部工具与服务
