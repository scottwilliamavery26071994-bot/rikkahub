...
class S3Sync(
    ...
    suspend fun backupToS3(config: S3Config) = withContext(Dispatchers.IO) {
        ...
        // 备份轮转（#999）：最多保留 MAX_BACKUPS 个，删除最旧的
        runCatching {
            val items = listBackupFiles(config)
            if (items.size > MAX_BACKUPS) {
                items.drop(MAX_BACKUPS).forEach { deleteS3BackupFile(config, it) }
                Log.i(TAG, "backupToS3: rotation removed ${items.size - MAX_BACKUPS} old backups")
            }
        }.onFailure { e ->
            Log.w(TAG, "backupToS3: rotation failed", e)
        }
    }
