...
fun ASRProviderConfigure(
    ...
    Column(
        ...
        FormItem(
            ...
            OutlinedTextField(
                value = when (setting) {
                    ...
                    is ASRProviderSetting.SystemAsr -> "系统语音识别 (免费)"
                },
                onValueChange = {},
                readOnly = true,
        ...
        when (setting) {
            ...
            is ASRProviderSetting.SystemAsr -> SystemAsrASRConfiguration(setting, onValueChange)
        }
    }
}
...
@Composable
private fun SystemAsrASRConfiguration(
    setting: ASRProviderSetting.SystemAsr,
    onValueChange: (ASRProviderSetting) -> Unit
) {
    FormItem(
        label = { Text("语言 (可选)") },
        description = { Text("留空用系统默认. 例: zh-CN 中文 / en-US 英文") }
    ) {
        OutlinedTextField(
            value = setting.language,
            onValueChange = { onValueChange(setting.copy(language = it)) },
    ...
    FormItem(
        label = { Text("提示语 (可选)") },
        description = { Text("系统识别弹窗/提示中显示的引导文字") }
    ) {
        OutlinedTextField(
            value = setting.prompt,
            onValueChange = { onValueChange(setting.copy(prompt = it)) },
